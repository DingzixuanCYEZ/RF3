...
import { AppView, Deck, Phrase, GlobalStats, BackupData, ActivityLog, DeckSessionLog } from './types';
import { StudySession } from './components/StudySession';
import { Importer } from './components/Importer';
import { DeckEditor } from './components/DeckEditor';
import { ExamSession } from './components/ExamSession';
import { Button } from './components/Button';
import { PlusCircle, BookOpen, Trash2, BrainCircuit, ListOrdered, CheckCircle2, XCircle, BarChart2, FolderInput, CopyPlus, X, Clock, Edit, AlertTriangle, Settings, Download, Upload, FileJson, Check, GraduationCap, Play, FileText, Hash } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    if (storedDecks) {
      try {
        ...
        const sanitized = Array.isArray(parsed) ? parsed.map((d: any) => ({
          ...
          sessionHistory: d.sessionHistory || [] // Ensure history array exists
        })) : [];
        setDecks(sanitized);
      } catch (e) {
        ...
        if (parsed.daily?.date !== today) {
          ...
          setStats({
             ...
             daily: {
               ...
               activities: parsed.daily?.activities || []
             }
          });
        }
      } catch (e) {
  ...
  // Runtime Daily Reset Check (Handles open tab across midnight)
  useEffect(() => {
    const checkReset = () => {
      const today = getTodayDate();
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Apply Data - Ensure decks have stat objects and sessionHistory
  ...
  // Handle Review Stat Update (Real-time updates to global stats and deck stats)
  const handleReviewStat = (deckId: string, mode: 'STUDY' | 'EXAM', phraseId: string, isCorrect: boolean) => {
    ...
    setStats(prev => {
      ...
      const currentActivities = prev.daily?.activities || [];
      
      // Update Activity Log
      const deckName = decks.find(d => d.id === deckId)?.name || 'Unknown Deck';
      let newActivities = [...currentActivities];
      const existingActivityIndex = newActivities.findIndex(a => a.deckId === deckId && a.mode === mode);

      if (existingActivityIndex >= 0) {
        newActivities[existingActivityIndex] = {
          ...newActivities[existingActivityIndex],
          count: newActivities[existingActivityIndex].count + 1,
          correct: newActivities[existingActivityIndex].correct + (isCorrect ? 1 : 0),
          timestamp: Date.now() // Update timestamp to show it's recent
        };
      } else {
        newActivities.push({
          deckId,
  ...
  // Called when a user exits a session (Study or Exam)
  const handleSessionComplete = (deckId: string, mode: 'STUDY' | 'EXAM', durationSeconds: number, correct: number, wrong: number) => {
    if (durationSeconds < 5 && correct + wrong === 0) return; // Ignore empty sessions
    ...
    setDecks(prev => prev.map(d => {
      if (d.id === deckId) {
        return {
          ...d,
          sessionHistory: [newLog, ...(d.sessionHistory || [])]
        };
      }
      return d;
    }));
    ...
    if (deck) {
      return (
        <StudySession
          ...
          onReview={(id, correct) => handleReviewStat(deck.id, 'STUDY', id, correct)}
          onTimeUpdate={handleTimeUpdate}
          onSessionComplete={(duration, correct, wrong) => handleSessionComplete(deck.id, 'STUDY', duration, correct, wrong)}
        />
      );
    }
  }
    ...
    if (deck) {
      return (
        <ExamSession
          ...
          onUpdateDeck={updateDeck}
          onExit={() => setView(AppView.DASHBOARD)}
          onReview={(id, correct) => handleReviewStat(deck.id, 'EXAM', id, correct)}
          onTimeUpdate={handleTimeUpdate}
          onSessionComplete={(duration, correct, wrong) => handleSessionComplete(deck.id, 'EXAM', duration, correct, wrong)}
        />
      );
    }
  }
